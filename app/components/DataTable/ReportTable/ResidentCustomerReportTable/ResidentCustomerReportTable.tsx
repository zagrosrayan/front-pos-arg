...
import { RESIDENT_CUSTOMER_REPORT_API } from '@/routes/api/customer'
...
import { UserResidentReportResponseProps } from '@/types/userTypes'
import { SMS_API } from '@/routes/api/sms'
import FormTextArea from '@/app/components/ui/FormTextArea'
import { toast } from 'react-toastify'

const ResidentCustomerReportTable = () => {
  ...
  const [selectedCustomer, setSelectedCustomer] =
    useState<UserResidentReportResponseProps | null>(null)
  const [selectedItems, setSelectedItems] = useState<
    UserResidentReportResponseProps[] | []
  >([])
  ...
  const handleOpenSmsDialog = (customer: UserResidentReportResponseProps) => {
  ...
  const handleSendSms = async (data: any) => {
    try {
      ...
      if (selectedCustomer?.Mobile) {
        data.phone = selectedCustomer?.Mobile
      } else {
  ...
  const handleDownload = async () => {
    try {
      ...
      const request = RESIDENT_CUSTOMER_REPORT_API.getAll()
      ...
      await downloadFile(
        ...
        selectedOption.has('pdf')
          ? 'resident_customer_report.pdf'
          : 'resident_customer_report.xlsx'
      )
    } catch (error) {
      console.error(error)
    } finally {
  ...
  const columns: ColumnsData<UserResidentReportResponseProps>[] = [
    {
      name: 'نام مشتری',
      uid: 'name',
      render: (customer: UserResidentReportResponseProps) => (
...

    {
      ...
      render: (customer: UserResidentReportResponseProps) => (
        <span>
          {Number(customer.Reserve).toLocaleString('fa-IR') ?? '-------'}
        </span>
      ),
    },
    {
      ...
      render: (customer: UserResidentReportResponseProps) => (
        <span>
          {Number(customer.Room).toLocaleString('fa-IR') ?? '-------'}
        </span>
      ),
    },
    {
      ...
      render: (customer: UserResidentReportResponseProps) => (
        <span>
          {Number(customer.balance).toLocaleString('fa-IR') ?? '-------'}
        </span>
      ),
    },
    {
      ...
      render: (customer: UserResidentReportResponseProps) => (
        <span>
          {Number(customer.total_purchased).toLocaleString('fa-IR') ??
            '-------'}
        </span>
      ),
    },
  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        <div className="space-y-5">
          ...
          >
            ...
            <FormSelect
              ...
              label={FOOD_LABEL}
            />
            <FormNumberInput name="reserve_number" label="شماره رزور" />
            <FormNumberInput name="room_number" label="شماره اتاق" />
      ...
      <DataTable
        columns={columns}
        apiMethods={RESIDENT_CUSTOMER_REPORT_API}
        key={currentKey}
                ...
                >
                  <div className="mb-3 flex flex-col gap-5">
                    ...
                    <div className="flex flex-col gap-2">
                      <span>نام : {selectedCustomer?.GuestName}</span>
                      <span>
                        شماره موبایل : {selectedCustomer?.Mobile ?? ''}
                      </span>
                    </div>
                  </div>
...
export default ResidentCustomerReportTable
